...
# [UPDATED] Ensure local modules can be imported when running as a script
...
try:
    ...
    from gennjoy import njoy_execution_engine
...
class Config:
    ...
    # Critical Files
    # Assumes these files are located in the package root
    XSDIR_TEMPLATE = BASE_DIR / "xsdir_mcnp5"
    XSDIR_MASTER = OUTPUT_ACE / "xsdir"
    TEMP_DICT_FILE = BASE_DIR / "temperature_index.json"
...
class TSLProcessor:
    ...
    def _load_temp_dict(self) -> Dict:
        ...
        if not Config.TEMP_DICT_FILE.exists():
            Logger.warn(f"Temperature dictionary not found at: {Config.TEMP_DICT_FILE}")
            Logger.warn("Using default temperature 293.6K for validation checks.")
            return {} # Return empty dict, validation will need to handle this
        try:
    ...
    def _setup_directories(self):
        ...
        if Config.OUTPUT_ACE.exists():
            ...
            try:
                 # Only clean files, preserve directory
                for item in Config.OUTPUT_ACE.iterdir():
                    if item.is_dir():
                        shutil.rmtree(item)
                    else:
                        item.unlink()
    ...
    def _validate_temperatures(self, element_t: str, requested_temps: List[float]) -> List[float]:
        ...
        # If dictionary is empty or element not found, we trust the input file (Permissive Mode)
        if not self.temp_dict or element_t not in self.temp_dict:
            return requested_temps
        ...
        for t in requested_temps:
            ...
            if any(abs(t - at) < 1e-5 for at in allowed_temps):
                valid_temps.append(t)
            else:
                Logger.warn(f"Temp {t}K not explicitly supported for {element_t} in JSON. Allowed: {allowed_temps}")
                # We skip it to be safe, or could append if we want to force try
        
        return valid_temps if valid_temps else requested_temps # Fallback to requested if filtering fails completely
    ...
    def execute(self):
        ...
        Logger.header("STARTING THERMAL SCATTERING PROCESSING")
...
def get_njoy_cmd():
    ...
    if not shutil.which(cmd) and not Path(cmd).exists():
        Logger.warn(f"NJOY executable '{cmd}' not found!")
    return cmd
...
if __name__ == "__main__":
    ...
    # [UPDATED] Default paths relative to package
    def_n_path = Config.BASE_DIR / "data" / "incident_neutron_endf"
    def_t_path = Config.BASE_DIR / "data" / "thermal_scattering_endf"
    
    # Attempt to show cleaner internal paths
    try:
        disp_n = f"[Internal] {def_n_path.relative_to(Config.BASE_DIR)}"
        disp_t = f"[Internal] {def_t_path.relative_to(Config.BASE_DIR)}"
    except:
        disp_n = str(def_n_path)
        disp_t = str(def_t_path)
    
    print("-" * 50)
    nd_n = input(f"Enter Incident Neutron data path (Default: {disp_n}): ").strip()
    nd_t = input(f"Enter Thermal Scattering data path (Default: {disp_t}): ").strip()
    
    # Resolve Paths
    abs_n = Path(nd_n).resolve() if nd_n else def_n_path
    abs_t = Path(nd_t).resolve() if nd_t else def_t_path
