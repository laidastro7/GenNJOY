...
class Config:
    ...
    # In the package structure, this directory IS the source directory
    SRC_DIR = BASE_DIR 
...
def main():
    ...
    try:
        if not Config.THERMAL_DIR.exists():
            print(f"{Fore.RED}[ERROR] Thermal directory missing: {Config.THERMAL_DIR}{Style.RESET_ALL}")
            print(f"{Fore.YELLOW}Tip: Run Option 1 to download libraries.{Style.RESET_ALL}")
            sys.exit(1)
    except Exception as e:
        print(f"{Fore.RED}[ERROR] Accessing thermal directory: {e}{Style.RESET_ALL}")
        sys.exit(1)
